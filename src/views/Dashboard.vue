...
<template>
  <div
    class="flex h-screen w-screen overflow-hidden bg-linear-to-br from-slate-950 via-slate-900 to-slate-950"
  >
    <!-- Sidebar -->
    <aside
      class="flex flex-col border-r border-white/5 bg-slate-900/80 backdrop-blur-xl w-full md:w-80 md:min-w-72 md:max-w-md shadow-2xl"
      :class="isChatOpen ? 'hidden md:flex' : 'flex'"
    >
      <!-- user header -->
      <div
        class="flex items-center justify-between border-b border-white/5 px-3 py-2.5 bg-linear-to-r from-indigo-600/15 to-purple-600/15"
      >
        <button
          @click="openProfileModal"
          class="flex items-center gap-2 min-w-0 hover:bg-white/10 rounded-lg px-1.5 py-1 transition-all duration-200 group"
        >
          <span
            class="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-linear-to-br from-indigo-500 to-purple-600 font-semibold text-sm text-white shadow-lg ring-2 ring-white/10"
          >
      ...
      <!-- lista chat -->
      <div
        class="flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-700 scrollbar-track-transparent"
      >
        <div v-if="filteredChats.length === 0" class="p-6 text-center">
          <svg
            class="h-10 w-10 mx-auto text-slate-600 mb-3"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
    ...
    <main
      ...
      :class="isChatOpen ? 'flex' : 'hidden md:flex'"
    >
      <RouterView v-if="isChatOpen" />

      <div
        v-else
        class="hidden md:flex h-full flex-col items-center justify-center space-y-4 text-center px-4"
      >
        <div class="relative">
          <div
            class="absolute inset-0 bg-indigo-500/20 blur-3xl animate-pulse"
          ></div>
          <span
            class="relative flex h-20 w-20 items-center justify-center rounded-2xl bg-linear-to-br from-indigo-500/20 to-purple-500/20 border border-white/10 shadow-2xl"
          >
            <svg
        ...
        <div class="space-y-2">
          ...
          <p class="text-sm text-slate-400 max-w-md">
            Seleziona una chat dalla lista o crea una nuova conversazione per
            iniziare a chattare.
          </p>
        </div>

        <div
          class="inline-flex items-center gap-2 rounded-full bg-linear-to-r from-green-500/10 to-emerald-500/10 border border-green-500/20 px-3 py-2 text-[11px] font-semibold text-green-300 shadow-lg"
        >
          <svg
            class="h-3 w-3"
            fill="none"
