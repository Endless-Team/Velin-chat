...
const {
  chats, // ✅ AGGIUNTO per debug
  selectedChat,
  currentMessages,
  sendEncryptedMessage,
  selectChat,
  loadUserChats, // ✅ AGGIUNTO questo!
} = useEncryptedChat();
...
const isLoading = ref(true); // ✅ AGGIUNTO per gestire il caricamento
...
onMounted(async () => {
  ...
  try {
    // ✅ PRIMA carica tutte le chat se non sono già caricate
    if (chats.value.length === 0) {
      console.log("📦 Caricamento chat da Firebase...");
      await loadUserChats();
      console.log("✅ Chat caricate:", chats.value.length);
    } else {
      console.log("✅ Chat già in memoria:", chats.value.length);
    }

    // ✅ POI seleziona la chat specifica
    console.log("⏳ Selezione chat con ID:", chatId);
    await selectChat(chatId);

    console.log("✅ Chat selezionata:", selectedChat.value);
    console.log("💬 selectedChatId:", selectedChat.value?.id);
    console.log("👥 participants:", selectedChat.value?.participants);
  } catch (error) {
  ...
  console.log("🔓 isKeysUnlocked:", isKeysUnlocked.value);
...
async function sendMessage() {
  ...
  console.log("📝 Messaggio:", message.value);
  console.log("🔓 Keys unlocked:", isKeysUnlocked.value);
  console.log("💬 Selected chat:", selectedChat.value);
  console.log("🆔 Selected chat ID:", selectedChat.value?.id);

  if (!message.value.trim() || !selectedChat.value || isSending.value) {
    console.log("❌ Condizioni non soddisfatte, esco");
    console.log("  - message.trim():", message.value.trim());
...
<template>
  <div class="flex h-screen w-screen overflow-hidden bg-slate-950">
    ...
    <!-- ✅ Mostra la chat solo quando è caricata -->
    <main v-else class="flex flex-1 flex-col min-w-0">
      <!-- Header Chat -->
      <header
