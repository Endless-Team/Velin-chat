...
class KeyStore {
  ...
  async initializeKeys(userId: string, password: string): Promise<UserKeys> {
    try {
      // Genera salt casuale (Uint8Array)
      ...
      // Esporta le chiavi
      const publicKeyJwk = await cryptoService.exportPublicKey(
        keyPair.publicKey
      );
      ...
      return {
        ...
        iv,
        salt: cryptoService.arrayBufferToBase64(salt.buffer as ArrayBuffer),
      };
    } catch (error) {
  ...
  async unlockKeys(userKeys: UserKeys, password: string): Promise<boolean> {
    try {
      // Converti salt da base64 ad ArrayBuffer, poi a Uint8Array
      const saltBuffer = cryptoService.base64ToArrayBuffer(userKeys.salt);
