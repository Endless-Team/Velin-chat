...
async function handleSave() {
  ...
  try {
    ...
    // ✅ 1. Aggiorna il profilo Firebase Auth
    await updateUserProfile(updates);

    // ✅ 2. Salva anche nel database Firestore
    ...
    await setDoc(
      ...
      { merge: true }
    ); // merge: true per non sovrascrivere altri campi
...
<template>
  <Modal @close="handleCancel">
    ...
    <!-- Footer -->
    <div
      class="flex items-center justify-end gap-3 border-t border-white/10 p-6"
    >
      <button
        @click="handleCancel"
        :disabled="isLoading"
